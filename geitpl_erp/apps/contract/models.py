...
class Contract(models.Model):
    ...
    user = models.ForeignKey(CustomUser, related_name="contracts",limit_choices_to={'is_active': True})
...
class PaySlip(models.Model):
    ...
    def send_email_notification(cls):
        ...
        paysleeps = cls.objects.filter(created_at=date.today())
        for paysleep in paysleeps:
            subject, from_email= 'Your salary for %s'%(paysleep.salary_month), 'info@geitpl.com'
            to = [paysleep.user.email]
            if paysleep.user.personal_email:
                to.append(paysleep.user.personal_email)
